...
```bash
{
  ...
  echo 'ExecStart=/home/foo/node/target/release/datahighway --chain CHAINNAME --bootnodes BOOTNODE --name YOURNODENAME --validator --unsafe-ws-external --unsafe-rpc-external --rpc-cors=all --rpc-methods=Unsafe --execution=native -lruntime=debug'
  echo '[Install]'
